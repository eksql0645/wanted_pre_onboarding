...
- API 구현 시 크게 막히는 것은 없었다. 모르는 부분에 대해서는 구글링이나 책을 통해 해결이 가능했다. 제일 어려웠던 부분은 초기설정이었다. 초기 설정은 프로젝트 제일 처음에 하다보니 기억이 희미해서 시간을 많이 소비했다. 초기 설정 이후에도 ESLint 에러가 발생했지만 개인프로젝트를 하면서 겪었던 에러여서 이 역시 노션에 작성해둔 글을 보고 해결했다.
